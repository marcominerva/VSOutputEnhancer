...
public class ParsedData
{
    ...
    private static readonly ConcurrentDictionary<Type, Func<Match, Span, ParsedData>> parsedDataCreators = new();

    private static Func<Match, Span, ParsedData> GetParsedDataCreator(Type parsedDataType)
        => parsedDataCreators.GetOrAdd(parsedDataType, CreateParsedDataCreator);
    ...
    private static Func<Match, Span, ParsedData> CreateParsedDataCreator(Type parsedDataType)
    {
        ...
        var enumParse = typeof(Enum).GetMethod("Parse", [typeof(Type), typeof(string), typeof(bool)]);
        var stringEquals = typeof(string).GetMethod("Equals", [typeof(string), typeof(string), typeof(StringComparison)]);
        ...
        var groupCollectionItem = typeof(GroupCollection).GetProperty("Item", [typeof(string)]);
        var convertChangeType = typeof(Convert).GetMethod("ChangeType", [typeof(object), typeof(Type)]);
        var spanStart = typeof(Span).GetProperty("Start");
        var spanConstructor = typeof(Span).GetConstructor([typeof(int), typeof(int)]);
        ...
        foreach (var property in propertiesToFill)
        {
            ...
            if (propertyValueType.IsEnum)
            {
                ...
                valueVarAssign = propertyValueType
                    .GetFields()
                    .SelectMany(f => f.GetCustomAttributes<EnumValueAttribute>().Select(a => new { Field = f, a.Value }))
                    .Aggregate(
                        valueVarAssign,
                        (current, value) =>
                        {
            ...
            var constructor = property.PropertyType.GetConstructor([propertyValueType, typeof(Span)]);
