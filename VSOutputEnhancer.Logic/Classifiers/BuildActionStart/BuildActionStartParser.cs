...
namespace Balakin.VSOutputEnhancer.Logic.Classifiers.BuildActionStart
{
    ...
    {
        public bool TryParse(SnapshotSpan span, out BuildActionStartData result)
        {
            ...
            const string Pattern = @"^(?:(?<BuildTaskId>\d+)>)?(?<FullMessage>------ (?<Action>.+) started: Project: (?<ProjectName>.*), Configuration: (?<Configuration>.*) ------)\r\n$";
